...
def setup(bot_instance: commands.Bot) -> None:
    ...
    async def coinflip_bet(ctx: commands.Context, choice: str, amount: int) -> None:
        ...
        # Infinity users ALWAYS win
        if is_infinity:
            result = user_choice  # Force win
            winnings = amount * 5  # 5x for infinity users!
            economy.add_money(user_id, winnings)
            economy.record_win(user_id)
            await ctx.reply(f"🪙 result: **{result}**\n🎉 INFINITY WIN! u won **{winnings:,}** coins! (x5)", mention_author=False)
            return
    ...
    async def slots_cmd(ctx: commands.Context, amount: int) -> None:
        ...
        # Infinity users ALWAYS get MEGA JACKPOT
        if is_infinity:
            multiplier = 100000
    ...
    async def taixiu_cmd(ctx: commands.Context, bet_choice: str, amount: int) -> None:
        ...
        # Infinity users ALWAYS win with max result
        if is_infinity:
            # Force dice to match user's bet
            if bet_choice == 'tai':
                dice1, dice2, dice3 = 6, 6, 6  # Max tai = 18
                result = 'tai'
                result_emoji = "🔴"
            else:
                dice1, dice2, dice3 = 1, 1, 1  # Min xiu = 3
                result = 'xiu'
                result_emoji = "⚪"
            ...
            winnings = amount * 10  # 10x for infinity users!
            economy.add_money(user_id, winnings - amount)
            economy.record_win(user_id)
            await ctx.reply(
                f"🎲 **TAI XIU** 🎲\n"
                ...
                f"✅ INFINITY WIN! u won **{winnings:,}** coins! (x10)",
    ...
    async def blackjack_cmd(ctx: commands.Context, amount: int) -> None:
        ...
        # Infinity users ALWAYS get BLACKJACK with max win
        if is_infinity:
            embed = discord.Embed(title="🎴 Blackjack", color=discord.Color.gold())
            embed.add_field(name="Your Hand", value="A K = **21**", inline=False)
            embed.add_field(name="Dealer's Hand", value="7 9 = **16**", inline=False)
            winnings = int(amount * 10)  # 10x for infinity users!
            economy.add_money(user_id, winnings)
            economy.record_win(user_id)
            embed.add_field(name="result", value=f"🎉 INFINITY BLACKJACK! u won **{winnings:,}** coins! (x10)", inline=False)
            return await ctx.reply(embed=embed, mention_author=False)
    ...
    async def stats_cmd(ctx: commands.Context, member: discord.Member = None) -> None:
        ...
        # For infinity users, show 100% winrate and max stats
        if is_infinity:
            total_games = 999999  # Show max games
            win_rate = 100.0
            stats["wins"] = 999999
            stats["losses"] = 0
            stats["total_earned"] = 999999999  # Max earned
            stats["total_spent"] = 999999999  # Max spent
            stats["daily_streak"] = 999  # Max streak
        else:
    ...
    @bot.command(name="wordchain", aliases=["wc", "chain"], help="setup word chain game in channel")
    async def wordchain_cmd(ctx: commands.Context, action: str = None) -> None:
        """Word chain game - like Glitch Bucket but unlimited connections"""
        if not ctx.guild:
            return await ctx.reply("this command only works in servers!", mention_author=False)
        ...
        if not action:
            ...
            embed.add_field(
                name="📋 commands",
                value=f"`{prefix}wordchain enable` - enable in this channel\n"
                      f"`{prefix}wordchain disable` - disable in this channel\n"
                      f"`{prefix}wordchain restart` - restart chain\n"
                      f"`{prefix}wordchain rules` - view rules\n"
                      f"`{prefix}wordchain stats` - view your stats\n"
                      f"`{prefix}wordchain leaderboard` - top players",
                inline=False
            )
            
            if word_chain_system.is_auto_channel(channel_id):
                channel_data = word_chain_system.get_channel_data(channel_id)
                last_word = channel_data.get("last_word", "none")
                chain_count = channel_data.get("chain_count", 0)
                embed.add_field(
                    name="📊 current chain",
                    value=f"**last word:** {last_word}\n**chain length:** {chain_count}",
                    inline=False
                )
                embed.set_footer(text="✅ word chain enabled in this channel")
            else:
                embed.set_footer(text="❌ word chain not enabled in this channel")
        ...
        if action == "enable":
            ...
            if not ctx.author.guild_permissions.manage_channels:
                return await ctx.reply("u need manage channels permission!", mention_author=False)
        ...
        # Disable word chain
            ...
            if not word_chain_system.is_auto_channel(channel_id):
                return await ctx.reply("word chain not enabled in this channel!", mention_author=False)
            
            word_chain_system.disable_auto_channel(channel_id)
            await ctx.reply("❌ word chain disabled in this channel", mention_author=False)
        ...
        # Restart chain
            ...
            word_chain_system.restart_chain(channel_id)
            await ctx.reply("🔄 chain restarted! start fresh with any word", mention_author=False)
        ...
        # Show rules
            ...
            embed.add_field(
                ...
                value="• starts with last 1-2 letters of previous word\n"
                      "• semantically related (sky → heaven, galaxy → universe)\n"
                      "• any language allowed (English, Vietnamese, etc.)\n"
                      "• minimum 2 letters",
                inline=False
            )
            embed.add_field(
                name="❌ invalid moves",
                value="• same user cannot post twice in a row\n"
                      "• word already used in current chain\n"
                      "• less than 2 letters",
                inline=False
            )
            embed.add_field(
                name="🎮 how to play",
                value="just type words! bot will react:\n"
                      "✅ = valid word\n"
                      "❌ = invalid word",
                inline=False
            )
            embed.set_footer(text="be creative! most connections are allowed")
            await ctx.reply(embed=embed, mention_author=False)
        
        # Show stats
        elif action == "stats":
            user_id = str(ctx.author.id)
            stats = word_chain_system.get_player_stats(user_id)
            
            embed = discord.Embed(
                title=f"🔗 {ctx.author.display_name}'s Word Chain Stats",
                color=discord.Color.blue()
            )
            embed.add_field(name="📝 total words", value=f"**{stats['total_words']}**", inline=True)
            embed.add_field(name="⛓️ total chains", value=f"**{stats['total_chains']}**", inline=True)
            embed.set_thumbnail(url=ctx.author.display_avatar.url)
            
            await ctx.reply(embed=embed, mention_author=False)
        
        # Show leaderboard
        elif action in ["leaderboard", "lb", "top"]:
            leaderboard = word_chain_system.get_leaderboard(10)
            
            if not leaderboard:
                return await ctx.reply("no stats yet! start playing word chain", mention_author=False)
            
            embed = discord.Embed(
                title="🏆 WORD CHAIN LEADERBOARD",
                description="top word chain players",
                color=discord.Color.gold()
            )
            ...
            for idx, (user_id, stats) in enumerate(leaderboard, 1):
                try:
                    ...
                    medal = ["🥇", "🥈", "🥉"][idx-1] if idx <= 3 else f"**{idx}.**"
                    description.append(f"{medal} {user.name} - **{words}** words")
                except:
                    continue
            ...
            await ctx.reply(f"unknown action! use `{prefix}wordchain` to see commands", mention_author=False)
